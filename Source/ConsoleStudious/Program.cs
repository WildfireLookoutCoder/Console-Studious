...
namespace ConsoleStudious
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            commands.Add(new Command("Session", false));
            ...
            // Recite phase: Go through all questions that have answers. Only show the preferred answer.
            // First show the whole question, with none of the answer. Sleep(3 seconds)
            // Then show the Question with the first 1/3 of the answer string. Sleep(3 seconds)
            // Then show the full answer. Sleep(3 seconds)

            // Review phase: For each question stored in the session:
            // Display question in full and prompt for user to type in a new answer as input
            // Display input beside question.answers.Where(a => a.preffered)
            // Ask user to indicate a preference between the two
        }
    }
}
